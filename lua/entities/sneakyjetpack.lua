	...
	hook.Add("Think", "test", function()
		hook.Add( "PlayerButtonDown", "PlayerButtonDownWikiExample", function( ply, button )
			if button == KEY_CAPSLOCK then
				if IsFirstTimePredicted() and ply:IsValid() and ply:HasWeapon('csg_jetpack') then 
					print('swapping to', !isActive)
					isActive = !isActive
				end
			end
		end)
...
function ENT:Initialize()
	...
	if SERVER then
		-- tasty -- self:SetModel( "models/themexicanjew/Jetpack.mdl" )
		self:InitPhysics()
